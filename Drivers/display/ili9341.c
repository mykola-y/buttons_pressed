...
void inline delay(unsigned int t) {
	for (; t > 0; t--) {
		__asm("nop");
	}
}
...
void LCD_Printf(const char *fmt, ...) {
	...
	while (*p) {
		if (*p == '\n') {
			...
			if(m_cursor_x == 0) {
				LCD_SetAddrWindow(m_cursor_x, m_cursor_y, m_cursor_x + m_width - 1, m_cursor_y + m_textsize * 8);
				LCD_Flood(m_textbgcolor, (long) TFTWIDTH * m_textsize * 8);
			}
			...
			if (m_wrap && (m_cursor_x > (m_width - m_textsize * 6))) {
				m_cursor_y += m_textsize * 8;
				m_cursor_x = 0;
//				LCD_SetAddrWindow(0, m_cursor_y, m_width - 1, m_cursor_y + m_textsize * 8);
//				LCD_Flood(m_textbgcolor, (long) TFTWIDTH * m_textsize * 8);
			}
		}
		p++;
 ...
 * \return void
 */
void LCD_ClearLines(uint8_t startLine, uint8_t numLines) {
	if (startLine < 0) startLine = 0;
	if (numLines < 1) numLines = 1;
	m_cursor_x = 0;
	m_cursor_y = startLine * m_textsize * 8;
	LCD_SetAddrWindow(0, m_cursor_y, m_width - 1, m_cursor_y + numLines * m_textsize * 8);
	LCD_Flood(m_textbgcolor, (long) TFTWIDTH * (long) numLines * m_textsize * 8);
}
